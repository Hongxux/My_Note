...
| 架构阶段                   | 核心问题                        | 后果                      | 解决方案演进             |
| ---------------------- | --------------------------- | ----------------------- | ------------------ |
| **单节点 Redis**​         | 服务器宕机、网络故障、进程崩溃             | **服务完全不可用**，数据无法读写      | 基础缓存，无法保证高可用       |
| **[[Redis主从\|主从复制]]**​ | 主节点故障后，**需要手动干预**将从节点提升为主节点 | **恢复时间过长（分钟/小时级）**，运维复杂 | 数据备份与读写分离，但故障切换不自动 |
| **Redis 哨兵**​          | **自动检测主节点故障**并**完成自动故障转移**​ | **秒级自动切换**，服务快速恢复，实现高可用 | ✅ **解决自动故障转移**     |
1. 哨兵集群自动**检测**到主节点下线。
    
2. 哨兵内部通过投票协议达成共识。
    
3. 自动**选择一个最优的从节点**并将其提升为新的主节点。
    
4. 自动**通知**其他从节点复制新的主节点。

| 功能              | 描述                                    | 实现机制                                          |
| --------------- | ------------------------------------- | --------------------------------------------- |
| **1. 监控**​      | 持续检查主、从节点是否正常运行。                      | 定期向所有实例发送 `PING`命令，根据响应判断状态。                  |
| **2. 自动故障转移**​  | 主节点故障时，自动将从节点提升为主节点，并让其他从节点复制新主节点。    | 使用Raft协议在哨兵实例间选举领导哨兵(最早发现主观下线的哨兵），由它执行故障转移操作。 |
| **3. 配置提供与通知**​ | 作为**服务发现**的权威源，客户端可查询哨兵以获取当前有效的主节点地址。 | 客户端连接至哨兵，通过命令获取最新主节点信息                        |
### Redis哨兵的实现原理
#### 1.监控
- 含义:持续检查主、从节点是否正常运行。
- 实现机制:
