...
2. **服务器验证并生成JWT**：服务器验证凭证有效后，会生成一个JWT。这个JWT的Payload中通常包含用户标识（如用户ID）、角色权限等信息，并设置一个过期时间（`exp`）。服务器使用一个保密密钥（用于HMAC算法）或一对公私钥（用于RSA算法）为这个JWT生成签名。
